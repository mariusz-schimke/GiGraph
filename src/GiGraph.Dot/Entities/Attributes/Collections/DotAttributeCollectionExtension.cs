...
public static class DotAttributeCollectionExtension
{
    ...
    /// <summary>
    ///     Checks if an attribute with the specified key exists in the collection, and returns its value as
    ///     <see cref="DotRankSpacingDefinition"/>. If the attribute is found, but its value cannot be cast nor converted to the returned
    ///     type, an exception is thrown.
    /// </summary>
    public static bool GetComplexValue(this DotAttributeCollection @this, string key, [MaybeNullWhen(false)] out DotRankSpacingDefinition value)
    {
        ...
        if (attribute.TryGetValueAs<int>(out var rankSpacingInt))
        {
            value = rankSpacingInt;
            return true;
        }

        if (attribute.TryGetValueAs<double>(out var rankSpacingDouble))
        {
            value = rankSpacingDouble;
            return true;
        }

        if (attribute.TryGetValueAs<double[]>(out var radialSpacing))
        {
            value = radialSpacing;
            return true;
        }
